...
import (
	...
	"github.com/aksgpt-ai/aksgpt/pkg/ai"
	aksgptserver "github.com/aksgpt-ai/aksgpt/pkg/server"
...
var ServeCmd = &cobra.Command{
	Use:   "serve",
	Short: "Runs aksgpt as a server",
	Long:  `Runs aksgpt as a server to allow for easy integration with other applications.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(configAI.Providers) == 0 {
			...
			temperature := func() float32 {
				env := os.Getenv("aksgpt_TEMPERATURE")
				if env == "" {
			...
			backend = os.Getenv("aksgpt_BACKEND")
			password := os.Getenv("aksgpt_PASSWORD")
			model := os.Getenv("aksgpt_MODEL")
			baseURL := os.Getenv("aksgpt_BASEURL")
			engine := os.Getenv("aksgpt_ENGINE")
			...
			if envIsSet {
				...
				color.Red("Error: AI provider not specified in configuration. Please run aksgpt auth")
				os.Exit(1)
			}
		}
		if aiProvider == nil {
		...
		if aiProvider.Name == "" {
			color.Red("Error: AI provider %s not specified in configuration. Please run aksgpt auth", backend)
			os.Exit(1)
		}
