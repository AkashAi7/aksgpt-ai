...
var removeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// verify dupplicate filters example: aksgpt filters remove Pod Pod
		...
		if len(filterNotFound) != 0 {
			color.Red("Filter(s) %s does not exist in configuration file. Please use aksgpt filters add.", strings.Join(filterNotFound, ", "))
			os.Exit(1)
		}
