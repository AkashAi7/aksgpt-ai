...
import (
	...
	"github.com/aksgpt-ai/aksgpt/pkg/analyzer"
	"github.com/aksgpt-ai/aksgpt/pkg/cache"
	"github.com/aksgpt-ai/aksgpt/pkg/common"
	"github.com/aksgpt-ai/aksgpt/pkg/custom"
	"github.com/aksgpt-ai/aksgpt/pkg/kubernetes"
...
func NewAnalysis(
	...
	if len(configAI.Providers) == 0 {
		return nil, errors.New("AI provider not specified in configuration. Please run aksgpt auth")
	}
	...
	if aiProvider.Name == "" {
		return nil, fmt.Errorf("AI provider %s not specified in configuration. Please run aksgpt auth", backend)
	}
	...
	if len(a.Filters) != 0 {
		...
		for _, filter := range a.Filters {
			if analyzer, ok := analyzerMap[filter]; ok {
				...
				a.Errors = append(a.Errors, fmt.Sprintf("\"%s\" filter does not exist. Please run aksgpt filters list.", filter))
			}
		}
		wg.Wait()
		return
